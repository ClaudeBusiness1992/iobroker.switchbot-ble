...
async function createBotObjects(adapter, data) {
    ...
    if (data.serviceData.model === 'H') {
    ...
    } else if (['T', 'i', 'w', '5', '4'].includes(data.serviceData.model)) {
        // SwitchBot "Meter" (Plus/Pro), "Indoor/Outdoor Thermo-Hygrometer" and "Meter Pro (CO2)"
        await adapter.createObjectNotExists(
            data.address + '.temperature', 'Temperature value',
            'number', 'value.temperature', false, data.serviceData.celsius, '°C');
        await adapter.createObjectNotExists(
            data.address + '.temperatureF', 'Temperature value (Fahrenheit)',
            'number', 'value.temperature', false, data.serviceData.fahrenheit, '°F');
        await adapter.createObjectNotExists(
        ...
        if (data.serviceData.model === '5') {
            // SwitchBot "Meter Pro (CO2)"
            await adapter.createObjectNotExists(
                data.address + '.co2', 'CO2 value', 'number', 'value', false, data.serviceData.co2, 'ppm');
        }
...
async function setAdvertisementData(adapter, data) {
    ...
    if (data.serviceData) {
        ...
        adapter.setStateConditional(data.address + '.deviceInfo.rssi', data.rssi, true, true);
        ...
        adapter.setStateConditional(data.address + '.deviceInfo.battery', data.serviceData.battery, true, true);
        ...
        if (data.serviceData.model === 'H') {
            ...
            const temperature = data.serviceData.celsius + Number(offsetTemperature);
            const temperatureF = data.serviceData.fahrenheit + Number(offsetTemperatureF);
            const humidity = data.serviceData.humidity + Number(offsetHumidity);
            adapter.setStateConditional(data.address + '.temperature', temperature, true, true);
            adapter.setStateConditional(data.address + '.temperatureF', temperatureF, true, true);
            adapter.setStateConditional(data.address + '.humidity', humidity, true, true);
            if (data.serviceData.model === '5') {
                // SwitchBot "Meter Pro (CO2)"
                adapter.setStateConditional(data.address + '.co2', data.serviceData.co2, true, true);
            }
        } else if ((data.serviceData.model === 'c') || (data.serviceData.model === '{')) {
            // SwitchBot "Curtain"
